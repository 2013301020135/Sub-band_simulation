...
def noise_from_parfile(par, **kwargs):
    ...
    To do: Add options for 'rfp' and 'dfp'.
...
if __name__ == '__main__':
    ...
    parser.add_argument('--skipfake', '--skip-fake', action='store_true', help='Skip fake ToA generation')
    parser.add_argument('--skipinject', '--skip-inject', action='store_true', help='Skip noise injection')
    ...
    args_keys = ['parfile', 'datadir', 'testd', 'comtd', 'reald', 'timd', 'resd', 'chaind', 'noised', 'maxha', 'rha',
                 ...
                 'ranidum', 'randnum', 'rlzno', 'strategy', 'skipfake', 'skipinject']
    ...
    for parfile, psrn in zip(par_files, psrnlist):
        ...
        if kw_args['nuhfb'] != 0 and not kw_args['skipfake']:
        ...
        if kw_args['nlb'] != 0 and not kw_args['skipfake']:
        ...
        if kw_args['nsb'] != 0 and not kw_args['skipfake']:
    ...
    if not kw_args['skipinject']:
        ...
        if kw_args['gwb']:
            print(f"GWB injecting: amplitude {kw_args['gwbamp']}, gamma {kw_args['gwbgamma']}, "
                  f"points {kw_args['gwbnpts']}")
            print(psrnlist)
            ...
            #                             timfile=f"{datadir + extradir + kw_args['timd'] + psrn}_injected.tim")
            #              for psrn in psrnlist]
            ltt.createGWB(psrobject, kw_args['gwbamp'], kw_args['gwbgamma'], npts=kw_args['gwbnpts'],
                          lmax=kw_args['lmax'], noCorr=kw_args['nocorr'], howml=kw_args['gwbhowml'],
                          turnover=kw_args['turnover'], f0=kw_args['gwbf0'], beta=kw_args['gwbbeta'],
                          power=kw_args['gwbpower'], seed=kw_args['randnum'])
            print(f"GWB injected: amplitude {kw_args['gwbamp']}, gamma {kw_args['gwbgamma']}, "
                  f"points {kw_args['gwbnpts']}")
