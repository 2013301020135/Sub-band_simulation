#!/usr/bin/env python
"""Run fake_ToAs, do MCMC fit, and draw plots for a series of parameter sets in parallel.
...
def build_task_params(**kwargs):
    ...
    for val in kwargs['values']:
        ...
        describe = fT.describe_name(**toa_params)
        ...
        fT.make_directories(toa_params['datadir'], toa_params['testd'], toa_params['comtd'], toa_params['reald'], True)
...
def extract_results(paras, **kwargs):
    ...
    for p in paras:
        s = {}
        p = fT.subband_strategy(**p)
        datad, pfiles, psrs = fT.psr_list(p['datadir'])
        ...
        extd = fT.make_directories(p['datadir'], p['testd'], p['comtd'], p['reald'])
...
def generate_toa(**kwargs):
    ...
    """
    start = time.time()
    extradir = fT.make_directories(kwargs['datadir'], kwargs['testd'], kwargs['comtd'], kwargs['reald'], make=True)
    command = ["python", kwargs['workd']+"fake_ToAs.py", "-d", kwargs['datadir'], "--timd", kwargs['timd'],
    ...
    if kwargs['skipfake']:
        command.extend(["--skipfake"])
    if kwargs['skipinject']:
        command.extend(["--skipinject"])
    if kwargs['wfp']:
        command.extend(["--wfp"])
    if kwargs['rfp']:
        command.extend(["--rfp"])
...
def mcmc_fit(**kwargs):
    ...
    """
    start = time.time()
    extradir = fT.make_directories(kwargs['datadir'], kwargs['testd'], kwargs['comtd'], kwargs['reald'])
...
def draw_plots(**kwargs):
    ...
    start = time.time()
    extradir = fT.make_directories(kwargs['datadir'], kwargs['testd'], kwargs['comtd'], kwargs['reald'])
    describe = fT.describe_name(**kwargs)
...
def execute_pipeline(**kwargs):
    ...
    try:
        if kwargs['refit'] is None:
            ...
            if kwargs['vary'] in ["refsig", "refgamma"] or (kwargs['vary'] == "rlzno" and kwargs['ranidum'] is None):
                ...
                logging.info("===Skip ToA injection===")
                if kwargs['faketoa'] is None:
                    logging.info("===Generate fake ToAs only once===")
                    # Clone and modify parameter values from input values for the fake ToA generation
                    fake_params = copy.deepcopy(kwargs)
                    fake_params['skipinject'] = True
                    config = VARY_PARAM_MAP[fake_params['vary']]
                    typeval = config['type'](fake_params['values'][0])
                    validate_parameter(config, typeval)
                    describe = fT.describe_name(**fake_params)
                    if fake_params['rlzno'] is not None:
                        fake_params['reald'] = str(fake_params['rlzno'])
                    fake_params[config['args_field']] = typeval
                    if config['in_log']:
                        fake_params[config['args_field']] = np.power(10, typeval)
                    fake_params['testd'] = f"{fake_params['testd']}_{fake_params['vary']}"
                    # Name the test directory with testd and the varying parameter
                    fake_params['comtd'] = f"{fake_params['comtd']}{describe}"
                    # Name the comment directory with comtd and the injected signals                    
                    fake_params['reald'] = f"realization_{fake_params['vary']}%{typeval}#{fake_params['reald']}"
                    # Name the realization directory with varying parameter and its value, plus the No. of realization
                    kwextd = fT.make_directories(fake_params['datadir'], fake_params['testd'], fake_params['comtd'],
                                                 fake_params['reald'], True)
                    logging.info(f"Fake ToA generating for: {fake_params['vary']} = {typeval}")  
                    generate_toa(**fake_params)  # Call monitored_parallel_execute?           
                else:
                    logging.info("===Use existing fake ToAs===")
                    kwextd = kwargs['faketoa']
                fakedir = kwargs['datadir'] + kwextd + kwargs['timd']
                logging.info("===Skip ToA simulation and inject noise for other params===")
                kwargs['skipfake'], kwargs['skipinject'] = True, False
                toa_params = build_task_params(**kwargs)                
                logging.info(f"Successfully constructed parameters lists for {len(toa_params)} tasks")
                # Construct parameters lists and record
                logging.info("===Copy fake ToAs to other folders===")
                for toapara in toa_params:
                    extradir = fT.make_directories(toapara['datadir'], toapara['testd'], toapara['comtd'],
                                                   toapara['reald'], make=True)
                    destdir = toapara['datadir'] + extradir + toapara['timd']
                    fT.check_directory(destdir)
                    if os.path.relpath(fakedir) != os.path.relpath(destdir):
                        shutil.copytree(fakedir, destdir, dirs_exist_ok=True)
                    for f in os.listdir(destdir):
                        if ('_%' in f or '_injected' in f) and os.path.isfile(os.path.join(destdir, f)):
                            os.remove(os.path.join(destdir, f))
                logging.info("===Copy fake ToAs finished===")    
                generate_results = monitored_parallel_execute(generate_toa, toa_params, "ToA progress")
            # Check results
            ...
            for tp in toa_params:
                # desc = fT.describe_name(**tp)
                extd = fT.make_directories(tp['datadir'], tp['testd'], tp['comtd'], tp['reald'])
                fT.check_directory(tp['datadir']+extd+tp['chaind'])
                if os.path.relpath(kwargs['datadir'] + kwargs['refit'] + f"/{tp['reald']}/") != os.path.relpath(tp['datadir']+extd):
                    copy_folder(kwargs['datadir'] + kwargs['refit'] + f"/{tp['reald']}/" + tp['resd'],
                                tp['datadir'] + extd + tp['resd'])
                    copy_folder(kwargs['datadir'] + kwargs['refit'] + f"/{tp['reald']}/" + tp['chaind'] + tp['noised'],
                                tp['datadir'] + extd + tp['chaind'] + tp['noised'])
                    shutil.copy(kwargs['datadir'] + kwargs['refit'] + f"/{tp['reald']}/paras_info.txt",
                                tp['datadir'] + extd + "/paras_info.txt")
                    shutil.copy(kwargs['datadir'] + kwargs['refit'] + f"/{tp['reald']}/simulate_command.txt",
                                tp['datadir'] + extd + "/simulate_command.txt")
                    shutil.copy(kwargs['datadir'] + kwargs['refit'] + f"/{tp['reald']}/simulate_out.txt",
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Pipeline for observational strategy research.'
                                                 ...
                                                 'Generate simulated ToAs for a sample of pulsars with fake_ToAs.'
    ...
    parser.add_argument('--faketoa', '--fake-toa', type=str, default=None,
                        help='Copy the fake ToAs in given test folder + comment folder + realization folder'
                             'and inject noise based on it')
    ...
    args_keys = ['parfile', 'datadir', 'timd', 'resd', 'chaind', 'noised', 'testd', 'comtd', 'reald', 'maxha', 'rha',
                 ...
                 'gwbhowml', 'nocorr', 'vary', 'values', 'workd', 'entd', 'ranidum', 'randnum', 'faketoa', 'refit', 
                 'ncore', 'maxobs', 'samp', 'niter', 'burn', 'rlzno', 'strategy', 'sumnam']
    kw_args = {key: getattr(args, key) for key in args_keys if hasattr(args, key)}
    datadir, par_files, psrnlist = fT.psr_list(datad=kw_args['datadir'], pf=kw_args['parfile'])
    ...
    if not args.gwb:
        kw_args['gwbnb'] = None
    kw_args = fT.subband_strategy(**kw_args)
    kw_args['skipfake'], kw_args['skipinject'] = False, False
    ...
    VARY_PARAM_MAP = {
        'rlzno': {'args_field': 'rlzno', 'type': int, 'in_log': False, 'valid_range': (1, 12)},
    ...
    try:
        ...
        fT.check_directory(kw_args['datadir'] + f"{kw_args['testd']}_{kw_args['vary']}")
