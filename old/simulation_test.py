...
if args.gwb:
    ...
    if not os.path.exists(datadir+args.resd):
        os.makedirs(datadir+args.resd)
...
if not os.path.exists(datadir+args.chaind):
    os.makedirs(datadir+args.chaind)

if not os.path.exists(datadir+args.noised):
    os.makedirs(datadir+args.noised)

for name in parlist:
    with open(f"{datadir}/{args.noised}/{name}.json", 'w') as file:
        file.writelines('{ \n')
