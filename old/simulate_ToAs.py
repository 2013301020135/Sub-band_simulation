...
# MODIFIED: added 'ranidum' parameter to pass per-pulsar seed
def tempo2_fake_simulate(pf, rmssub, subfreq, subbw, ranidum=None, **kwargs):
    ...
    :param ranidum: random seed for tempo2 fake (negative seven-digit integer), per pulsar
    """
    ...
    # MODIFIED: use ranidum if provided, else do not add -idum
    if ranidum is not None:
        command.extend(["-idum", str(ranidum)])
...
def save_paras(**kwargs):
    ...
    with open(infotxt, "w") as f:
        ...
        # MODIFIED: log the per-pulsar ranidum seeds (if any)
        if kwargs.get('ranidum_list') is not None:
            f.writelines(f"Per-pulsar tempo2 random seeds: {kwargs['ranidum_list']}\n")
        else:
            f.writelines(f"Per-pulsar tempo2 random seeds: None")
        f.writelines(f"Random number seed for GWB: {kwargs['randnum']}\n" if kwargs['randnum'] is not None else "")
        if kwargs['wfp']:
...
# MODIFIED: added 'ranidum' parameter for per-pulsar seed
def inject_noise(psrobj, par, ranidum=None, **kwargs):
    ...
    :param par: the par file for the pulsar to inject noise
    :param ranidum: per-pulsar seed (negative integer) to be used as -(ranidum) for RN and DM
    ...
    if kwargs['rn']:
        ...
        # MODIFIED: use -ranidum as seed (if ranidum is not None)
        seed_for_rn = -ranidum if ranidum is not None else None
        ltt.add_rednoise(psrobj, kwargs['rnamp'], kwargs['rngamma'],
                         components=kwargs['rnc'], tspan=kwargs['rntspan'], seed=seed_for_rn)
    ...
    if kwargs['dmn']:
        ...
        # MODIFIED: use -ranidum as seed (if ranidum is not None)
        seed_for_dm = -ranidum if ranidum is not None else None
        # ltt.add_dm(psrobj, kwargs['dmnamp'], kwargs['dmngamma'],
        #            components=kwargs['dmnc'], seed=seed_for_dm)
...
if __name__ == '__main__':
    ...
    # MODIFIED: changed ranidum to accept multiple integers (one per pulsar)
    parser.add_argument('--ranidum', '--random-tempo2-seed', type=int, nargs='*', default=None,
                        help='Specify random tempo2 seed(s) for each pulsar. If not provided, auto-generate negative seven-digit integers.')
    parser.add_argument('--randnum', '--random-number-seed', type=int, default=None,
                        help='Specify random number seed for GWB only.')
    ...
    # MODIFIED: generate per-pulsar ranidum list
    npsr = len(psrnlist)
    if args.ranidum is not None:
        # User provided a list; check length
        if len(args.ranidum) < npsr:
            raise ValueError(f"Number of ranidum values ({len(args.ranidum)}) does not cover number of pulsars ({npsr})")
        ranidum_list = args.ranidum[:npsr]
    else:
        # Auto-generate negative seven-digit integers (between -9999999 and -1000000)
        # Use numpy's default random generator
        rng = np.random.default_rng()
        ranidum_list = rng.integers(-9999999, -999999, size=npsr)  # high is exclusive, so -999999 gives -1000000 min
        # Ensure they are exactly seven digits (values are already in range)
        # Convert to list of ints
        ranidum_list = ranidum_list.tolist()
    # Store in kw_args for logging
    kw_args['ranidum_list'] = ranidum_list

    info = save_paras(**kw_args)

    tppsr = []
    # MODIFIED: iterate with index to get corresponding ranidum
    for idx, (parfile, psrn) in enumerate(zip(par_files, psrnlist)):
        ranidum_val = ranidum_list[idx]
        timlines = ["FORMAT 1 \n"]
        ...
        if kw_args['nuhfb'] != 0:
            ...
            for j, rms_sub in enumerate(rms_uhf):
                # MODIFIED: pass ranidum_val to tempo2_fake_simulate
                tempo2_fake_simulate(parfile, rms_sub, U_Band.subfreq[j], U_Band.subbw,
                                     ranidum=ranidum_val, **kw_args)
        ...
        if kw_args['nlb'] != 0:
            ...
            for j, rms_sub in enumerate(rms_l):
                # MODIFIED: pass ranidum_val
                tempo2_fake_simulate(parfile, rms_sub, L_Band.subfreq[j], L_Band.subbw,
                                     ranidum=ranidum_val, **kw_args)
        ...
        if kw_args['nsb'] != 0:
            ...
            for j, rms_sub in enumerate(rms_s):
                # MODIFIED: pass ranidum_val
                tempo2_fake_simulate(parfile, rms_sub, S_Band.subfreq[j], S_Band.subbw,
                                     ranidum=ranidum_val, **kw_args)
    ...
    # MODIFIED: now loop with idx to pass ranidum_val to inject_noise
    for idx, (parfile, psrn, psr) in enumerate(zip(par_files, psrnlist, tppsr)):
        ranidum_val = ranidum_list[idx]
        # MODIFIED: pass ranidum_val
        psrinject = inject_noise(psr, parfile, ranidum=ranidum_val, **kw_args)
    ...
    if kw_args['gwb']:
        ...
        #                             timfile=f"{datadir + extradir + kw_args['timd'] + psrn}_injected.tim")
        #              for psrn in psrnlist]
        # MODIFIED: use randnum for GWB seed (unchanged)
